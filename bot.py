...
TOKEN = '8404886467:AAEapmfllagU8C9rZhAHYWkK-iFprmKcgXY'  # Token bot kamu
CHANNEL = '@pejuanglendirdo'  # Channel kamu
...
async def lihat_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if member.status in ['member', 'administrator', 'creator']:
            ...
            keyboard = [[InlineKeyboardButton("JOIN CHANNEL SEKARANG", url=f"https://t.me/{CHANNEL[1:]}")]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await update.message.reply_text("Belum join channel nih.\nKlik tombol untuk join dulu!", reply_markup=reply_markup)
    except Exception as e:
        await update.message.reply_text("Error: Bot harus jadi admin di channel!")
